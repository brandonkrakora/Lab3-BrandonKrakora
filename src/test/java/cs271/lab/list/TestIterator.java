...
public class TestIterator {
  ...
  public void testRemove() {
    ...
    while (i.hasNext()) {
      if (i.next() == 77) {
        i.remove(); // Completed what happens if you use list.remove(Integer.valueOf(77))?
      }
    }
    // Completed using assertEquals and Arrays.asList, express which values are left in the list
  ...
  public void testAverageValues() {
    ...
    double sum = 0;
    int n = 0;
    // Completed use an iterator and a while loop to compute the average (mean) of the values
    final Iterator<Integer> i = list.iterator();
    while (i.hasNext()) {
      sum += i.next();
      n++;
